...
const zhRouteTranslations = {
  optimizeRoute: '最佳化路線', refreshRoute: '重新整理路線', routeStale: '路線需要重新整理', routeReady: '路線已更新', routeUnavailable: '找不到路線。請保留此順序或手動選擇交通方式。', routeDemo: '示範模式無法使用路線最佳化，請登入後使用 Google Routes。', dayDefault: '使用當日預設', publicTransport: '大眾運輸', walk: '步行', bike: '自行車', car: '開車', travelTo: '前往 {name}', routeDuration: '{minutes} 分鐘', routeDistance: '{distance} 公里', routeDetails: '路線詳情', routeLoading: '正在規劃路線…', routeMode: '交通方式', stayAt: '住宿地點', manageTimes: '管理時間', hideTimeManagement: '隱藏時間管理', routeError: '路線更新失敗', plannedStop: '安排停靠點', lunchDinner: '午餐／晚餐 — 尚未決定', coffeeBreak: '咖啡休息 — 尚未決定', freeTime: '自由時間', customStop: '自訂停靠點', choosePlace: '選擇景點',
} as const;
...
const translations = {
  en: {
    ...
    placesOfInterest: 'Places of interest', placesCount: '{shown} of {total} places', searchPlaceRegion: 'Search a place or region', searchPlaces: 'Search places', noPlacesFilter: 'No places match this filter.', all: 'All', landmark: 'Landmark', nature: 'Nature', culture: 'Culture', food: 'Food', shopping: 'Shopping', relaxation: 'Relaxation',
    ...
    deletePlaceQuestion: 'Delete place?', removePlaceConfirm: 'Remove {name} from the map and every itinerary day?', cancel: 'Cancel',
    removeDayQuestion: 'Remove day?', removeDayConfirm: 'Remove Day {number}? Its {count} stops will be moved to Unscheduled.',
    placeUpdated: 'Place updated', placeSaved: '{name} was saved.', placeAdded: 'Place added', placeReady: '{name} is ready to schedule.', placeRemoved: 'Place removed', placeDeleted: '{name} was deleted.', dayRemoved: 'Day removed', stopsMoved: '{count} stops moved to Unscheduled.', itineraryCopied: 'Itinerary copied', itineraryCopiedMessage: 'Plain text itinerary copied to your clipboard.', copyFailed: 'Copy failed', clipboardDenied: 'Your browser did not allow clipboard access.', demoRestored: 'Demo restored', demoRestoredMessage: 'The sample Taiwan trip has been reset.',
...
export function categoryLabel(t: I18nValue['t'], category: string) {
  const key = ({ Landmark: 'landmark', Nature: 'nature', Culture: 'culture', Food: 'food', Shopping: 'shopping', Relaxation: 'relaxation' } as Record<string, TranslationKey>)[category];

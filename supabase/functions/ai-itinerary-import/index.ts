...
const corsHeaders = { 'Access-Control-Allow-Origin': Deno.env.get('AI_IMPORT_ALLOWED_ORIGIN') ?? '*', 'Access-Control-Allow-Headers': 'authorization, apikey, content-type', 'Access-Control-Allow-Methods': 'POST, OPTIONS', Vary: 'Origin' };
...
function fail(code: string, message: string, status: number, requestId: string) { return json({ code, message, requestId, retryable: status >= 500 }, status); }
...
Deno.serve(async (request) => {
  ...
  if (request.method !== 'POST') return fail('METHOD_NOT_ALLOWED', 'Use POST.', 405, requestId);
  const authorization = request.headers.get('Authorization');
  if (!authorization?.startsWith('Bearer ')) return fail('AUTH_REQUIRED', 'Sign in to use AI import.', 401, requestId);
  let body: Record<string, unknown>;
  try { body = await request.json(); } catch { return fail('INVALID_SOURCE', 'Request body must be JSON.', 400, requestId); }
  const source = body.source as { type?: unknown; content?: unknown } | undefined;
  if (source?.type !== 'text' || typeof source.content !== 'string') return fail('INVALID_SOURCE', 'Only pasted text is supported.', 400, requestId);
  const content = normalizeText(source.content);
  if (content.length < 30) return fail('INVALID_SOURCE', 'Paste at least 30 characters of travel content.', 400, requestId);
  if (content.length > maxCharacters) return fail('SOURCE_TOO_LARGE', 'Pasted content is too long.', 413, requestId);

  const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
  const anonKey = Deno.env.get('SUPABASE_ANON_KEY')!;
  const userClient = createClient(supabaseUrl, anonKey, { global: { headers: { Authorization: authorization } } });
  const { data: userData } = await userClient.auth.getUser();
  if (!userData.user) return fail('AUTH_REQUIRED', 'Sign in to use AI import.', 401, requestId);
  const { data: accessibleTrips, error: tripError } = await userClient.from('trip_plans').select('user_id').limit(1);
  if (tripError || !accessibleTrips?.length) return fail('FORBIDDEN', 'Trip access is unavailable.', 403, requestId);
  // Browser roles cannot read usage rows; use a service client only for the quota ledger.
  const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
  if (!serviceKey) return fail('INTERNAL_ERROR', 'AI import is not configured.', 500, requestId);
  ...
  if ((serviceCount ?? 0) >= dailyLimit) return fail('AI_IMPORT_LIMIT_REACHED', 'Daily AI import limit reached.', 429, requestId);
  ...
  try {
    ...
    if (!providerResponse.ok) return fail(providerResponse.status === 429 ? 'MODEL_RATE_LIMITED' : 'MODEL_UNAVAILABLE', 'AI provider is temporarily unavailable.', 503, requestId);
    ...
    try { parsed = JSON.parse(raw ?? ''); } catch { return fail('AI_RESPONSE_INVALID', 'AI returned an invalid draft.', 502, requestId); }
    const candidates = (parsed.places ?? []).slice(0, 30).map(safeCandidate).filter((item): item is Candidate => item !== null);
    if (!candidates.length) return fail('AI_RESPONSE_INVALID', 'AI returned no valid travel places.', 502, requestId);
    ...
    return fail('INTERNAL_ERROR', 'Could not create an AI draft.', 500, requestId);
  }
});
